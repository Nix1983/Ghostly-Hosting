...
remove_fail2ban() {
  ...
  echo -e " Removed Fail2Ban configuration, binaries and Python modules."
}

intsall_fail2ban(){
    echo -e "\n \e[1mInstalling Fail2Ban (security)...\e[0m"
  ...
  if [[ ! -d /etc/fail2ban ]]; then
    echo -e " \e[33mFail2Ban config missing – repairing broken installation (Ubuntu 20 workaround)...\e[0m"
...
show_f2b_explanation() {
  ...
  echo "    Last ban = Timestamp and age of the last ban"
  echo "    Settings = Jail config like bantime, maxretry etc."
...
add_ip_to_whitelist() {
  ...
  if [[ -n "$current_section" ]]; then
    if [[ "$current_section" == "$SECTION" ]]; then
      echo -e "  IP \e[1;33m$IP\e[0m is already in [\e[1m$SECTION\e[0m]."
      return 0
    else
...
clear_fail2ban_whitelist() {
  ...
  if [[ ${#entries[@]} -eq 0 ]]; then
    echo -e "\n  No entries found in whitelist."
    return 1
  fi

  echo -e "\n  This will remove \e[1m${#entries[@]}\e[0m entries from the whitelist."
...
clear_fail2ban_blocklist() {
  ...
  if [[ ${#entries[@]} -eq 0 ]]; then
    echo -e "\n  No entries found in blocklist."
    return 1
  fi

  echo -e "\n  This will remove \e[1m${#entries[@]}\e[0m entries from the blocklist."
  read -rp "   Are you sure you want to proceed? [y/N]: " confirm
...
show_f2b_menu() {
  ...
  if ! systemctl is-active --quiet fail2ban; then
    echo -e "\n  \e[1;33mFail2Ban is installed but not running.\e[0m"
  ...
  while true; do
    clear
    echo -e "\n\e[1m  Fail2Ban Management"
    echo    "────────────────────────────────────────────────────────────"

    echo -e "\n 1)  Show status                2)  Manage whitelist"
    echo -e "\n 3)  Show explanation           4)  Manage blocklist"
