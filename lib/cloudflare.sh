...
_upsert_dns_record() {
  ...
  if [[ -n "$id" ]]; then
    ...
    printf "  Updating %s-record: \033[36m%s -> %s\033[0m\n" "$type" "$name" "$content"
  else
    method="POST"
...
toggle_cloudflare_proxy() {
  ...
  for record_type in "${types[@]}"; do
    ...
    if ! echo "$response" | jq -e '.result | length > 0' >/dev/null 2>&1; then
      echo -e "  No $record_type record found or invalid response."
      continue
    fi
  ...
  if [[ "$has_change" != true ]]; then
    echo -e "  \e[33mNo records updated – nothing toggled.\e[0m"
    return 1
  fi
...
delete_cloudflare_dns_records() {
  ...
  if [[ "$found_any" == false ]]; then
    echo -e " No A/AAAA DNS records found for \e[2m$HOSTNAME_FQDN\e[0m — skipping."
  else
  ...
  while true; do
    ...
    while true; do
      ...
      if [[ "$root_taken" != true ]]; then
        ...
        printf " 3) Back  Go back to zone selection\n"
        print_line
        printf " Your choice [1–3]: "
      else
        printf " 2) Back  Go back to zone selection\n"
        print_line
      ...
      case "$sub_choice" in
        1)
          while true; do
            printf "  Enter subdomain (e.g. \033[36mapp\033[0m): "
            read -r subname
            subname=${subname,,}
            ...
            if grep -q -Fx "$full_fqdn" <<< "$used_names"; then
              ...
              printf "  \033[33mSubdomain already in use:\033[0m \033[36m%s -> \033[36m%s\033[0m\n" "$full_fqdn" "$ip"
...
setup_cloudflare_dns_for_blazor() {
  ...
  printf "\n  \033[1mCloudflare DNS Setup for GhostlyHosting\033[0m\n"
  printf "────────────────────────────────────────────────────────────\n"
  ...
  printf "     For development, it's recommended to \033[33mdisable\033[0m the proxy.\n"
  ...
  if [[ -n "$SERVER_IPv6" ]]; then
    ...
    echo -e "  \033[2mNo IPv6 detected – skipping AAAA record.\033[0m"
  fi
  ...
  for record_type in A AAAA; do
    ...
    echo "$response" | jq -c '.result[]' | while read -r record; do
      ...
      [[ "$proxied" == "true" ]] && proxy_icon=" via CF" || proxy_icon=" direct"
...
delete_all_cloudflare_dns_records_for_server() {
  ...
  if [[ "$zone_count" -eq 0 ]]; then
    echo -e " No zones found in your Cloudflare account."
    print_press_any_key
    return 0
  fi
