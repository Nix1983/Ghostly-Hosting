...
_check_certificate_validity() {
  ...
  mapfile -t san_entries < <(openssl x509 -noout -ext subjectAltName -in "$path" 2>/dev/null \
    | grep -Eo 'DNS:[^, ]+' | cut -d: -f2)
  ...
  while IFS= read -r expected_domain; do
    ...
    if ! printf '%s\n' "${san_entries[@]}" | grep -Fxq "$expected_domain"; then
      missing+=("$expected_domain")
    fi
  done < <(_expected_certificate_domains)

  if [[ ${#missing[@]} -gt 0 ]]; then
    echo "⚠️  Certificate SAN is missing required domains: ${missing[*]}"
    return 1
  fi

  echo "✅ Valid certificate found for domains: $(printf '%s ' "${san_entries[@]}")"
  return 0
}
...
_obtain_or_verify_certificate() {
  ...
  while IFS= read -r domain_entry; do
    certbot_domains+=(-d "$domain_entry")
  done < <(_expected_certificate_domains)

  if ! certbot certonly --standalone "${certbot_domains[@]}" --cert-name "$HOSTNAME_FQDN" --email "admin@$DOMAIN" --non-interactive --agree-tos; then
