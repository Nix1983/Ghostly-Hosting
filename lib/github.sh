...
validate_github_token() {
  ...
  declare -f _safe_log >/dev/null 2>&1 && _safe_log debug "validate_github_token" "Validating GitHub API token"
  ...
  if [[ "$status" == "200" ]]; then
    declare -f _safe_log >/dev/null 2>&1 && _safe_log info "validate_github_token" "GitHub token validated successfully"
    return 0
  else
    declare -f _safe_log >/dev/null 2>&1 && _safe_log error "validate_github_token" "Token validation failed with status $status. Response: $body"
    return 1
  fi
}
...
check_github_env_vars() {
  ...
  declare -f _safe_log >/dev/null 2>&1 && _safe_log debug "check_github_env_vars" "GitHub environment validated for user: $GITHUB_API_USER"
  return 0
}
