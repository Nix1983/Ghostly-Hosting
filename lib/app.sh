...
_redeploy_blazor_app() {
  ...
  echo -e "\n \e[1mStopping service:\e[0m \e[36m$service_name\e[0m"
  systemctl stop "$service_name" 2>/dev/null || echo " Could not stop service."
...
delete_app() {
  ...
  echo "  Cloudflare DNS entry:  $domain"
  echo -e "\n This includes all logs and backups inside the app folder!"
  ...
  echo -e "\n \e[1mStopping and disabling service:\e[0m \e[36m$service\e[0m"
  ...
  if [[ -d "$exec_dir" ]]; then
    ...
    echo -e "  App folder not found: \e[2m$exec_dir\e[0m"
  fi

  echo -e "\n \e[1mRemoving Nginx config:\e[0m \e[36m$HOSTNAME_FQDN\e[0m"
  ...
  if nginx -t &>/dev/null; then
    ...
    echo -e "  \e[33mNginx config test failed – please check manually.\e[0m"
  fi

  echo -e "\n \e[1mDeleting SSL certificate (Certbot)...\e[0m"
  delete_certbot_certificate

  echo -e "\n \e[1mDeleting Cloudflare DNS records...\e[0m"
  ...
  if [[ -n "$CLOUDFLARE_API_TOKEN" && -n "$CLOUDFLARE_API_BASE" && -n "$ZONE_ID" && -n "$HOSTNAME_FQDN" ]]; then
    if delete_cloudflare_dns_records; then
      ...
      echo -e " \e[33mCloudflare DNS deletion failed for $HOSTNAME_FQDN.\e[0m"
      echo -e " Please check manually in the Cloudflare dashboard."
    fi
  else
    echo -e " \e[33mMissing CLOUDFLARE_API_TOKEN, ZONE_ID or HOSTNAME_FQDN – cannot delete DNS.\e[0m"
...
check_for_app_update() {
  ...
  if [[ "$ref_type" == "tag" ]]; then
    echo -e "\n  \e[33mThis app was deployed from a Git tag.\e[0m"
...
stop_app_service() {
  ...
  read -r confirm
  if [[ "$confirm" =~ ^[Yy]$ ]]; then
    systemctl stop "$service" && echo " Stopped." || echo -e " \e[31mFailed to stop.\e[0m"
    sleep 1
  fi
}
...
show_app_details_menu() {
  ...
  dns_summary="A: $( [[ "$has_a" -eq 1 ]] && echo yes || echo no )  AAAA: $( [[ "$has_aaaa" -eq 1 ]] && echo yes || echo no )"
  ...
  while true; do
   ...
   if [[ "$ref_type" == "branch" ]]; then
    ...
    printf " %-18s \e[36m%-22s\e[0m    %-17s \e[36m%-10s\e[0m\n" "Memory Usage:" "$ram_size" "Uptime:" "$uptime"
    printf " %-18s \e[36m%-23s\e[0m    %-17s \e[36m%-20s\e[0m\n" "SSL Certificate:" "$ssl_status" "SSL Auto Renew:" "$auto_renew"
