...
### Version Storage
- The current version is stored in the `lib/VERSION` file
...
- When a PR is merged to `main` or `master`, the GitHub Actions workflow `.github/workflows/version-increment.yml` automatically:
  1. Reads the current version from the `lib/VERSION` file
  2. Increments the PATCH version by 1
  3. Updates the `lib/VERSION` file
...
### Files
- `lib/VERSION` - Stores the current version number
...
1. Edit the `lib/VERSION` file directly
2. Commit the change
